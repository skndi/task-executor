...
vec3 raytrace(const Ray &r, Instancer &prims, int depth = 0) {
  ...
  if (prims.intersect(r, 0.001f, FLT_MAX, data)) {
    ...
    if (depth < MAX_RAY_DEPTH &&
        data.material->shade(r, data, attenuation, scatter)) {
    ...
    if (idx >= width * height) {
      if (completedThreads.fetch_add(1) == threadCount - 1) {
        ...
        const int success = stbi_write_png(
            resultImage.c_str(), width, height, PNGImage::componentCount(),
            png.data.data(), sizeof(PNGImage::Pixel) * width);
        assert(success == 1);
        return true;
      }
    }
...
void sceneExample(Scene &scene) {
  ...
  SharedPrimPtr mesh(new TriangleMesh(
      MESH_FOLDER "/cube.obj", MaterialPtr(new Lambert{Color(1, 0, 0)})));
  ...
  const float r = 0.6f;
  scene.addPrimitive(PrimPtr(new SpherePrim{
      vec3(2, 0, 0), r, MaterialPtr(new Lambert{Color(0.8, 0.3, 0.3)})}));
  scene.addPrimitive(PrimPtr(new SpherePrim{
      vec3(0, 0, 2), r, MaterialPtr(new Lambert{Color(0.8, 0.3, 0.3)})}));
  scene.addPrimitive(PrimPtr(new SpherePrim{
      vec3(0, 0, 0), r, MaterialPtr(new Lambert{Color(0.8, 0.3, 0.3)})}));
}
...
void sceneManyHeavyMeshes(Scene &scene) {
  ...
  auto getRandomMaterial = [instanceMaterials,
                            materialCount]() -> SharedMaterialPtr {
    const int rng = int(randFloat() * materialCount);
    return instanceMaterials[rng];
  };

  SharedPrimPtr mesh(new TriangleMesh(
      MESH_FOLDER "/dragon.obj", MaterialPtr(new Lambert{Color(1, 0, 0)})));
  Instancer *instancer = new Instancer;

  instancer->addInstance(mesh, vec3(0, 2.5, -count + 1), 0.08f,
                         getRandomMaterial());
...
void sceneHeavyMesh(Scene &scene) {
  ...
  scene.addPrimitive(PrimPtr(
      new TriangleMesh(MESH_FOLDER "/dragon.obj",
                       MaterialPtr(new Lambert{Color(0.2, 0.7, 0.1)}))));
}

typedef void (*SceneCreator)(Scene &);

struct Renderer : TaskSystem::Executor {
  Renderer(std::unique_ptr<TaskSystem::Task> taskToExecute)
  ...
  virtual ExecStatus ExecuteStep(int threadIndex, int threadCount) {
    return scene.renderStep(threadIndex, threadCount) ? ExecStatus::ES_Stop
                                                      : ExecStatus::ES_Continue;
  };
...
TaskSystem::Executor *
ExecutorConstructorImpl(std::unique_ptr<TaskSystem::Task> taskToExecute) {
  return new Renderer(std::move(taskToExecute));
}

IMPLEMENT_ON_INIT() { ts.Register("raytracer", &ExecutorConstructorImpl); }
